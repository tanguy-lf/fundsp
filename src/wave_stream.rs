...
pub struct WaveStream48 {
    ...
    len: usize,
    /// Theoric length of the wave in samples.
    theoric_len: Option<usize>,
    /// Is the wave fully set_loaded
    loaded: bool,
...
impl Clone for WaveStream48 {
    fn clone(&self) -> Self {
        Self {
            vec: self.vec.clone(),
            sr: self.sr,
            len: self.len,
            theoric_len: self.theoric_len,
            loaded: self.loaded,
            slice: Slice::new(),
        }
    }
...
impl WaveStream48 {
    ...
    /// ```
    pub fn new(channels: usize, sample_rate: f64, theoric_len: Option<usize>) -> Self {
    ...
    pub fn duration(&self) -> f64 {
        let len = if !self.loaded{
            self.theoric_len.unwrap_or(self.len)
        }else{
            self.len
        } as f64;
    ...
    pub fn render(sample_rate: f64, duration: f64, node: &mut dyn AudioUnit48) -> Self {
        ...
        wave.len = length;
        let mut i = 0;
        let mut buffer = Self::new(node.outputs(), sample_rate, None);
    ...
    pub fn filter(&self, duration: f64, node: &mut dyn AudioUnit48) -> Self {
        ...
        let mut i = 0;
        let mut input_buffer = Self::new(self.channels(), self.sample_rate(), None);
        let mut reusable_input_slice = Slice::<[f48]>::with_capacity(self.channels());
        let mut output_buffer = Self::new(node.outputs(), self.sample_rate(), None);
    ...
    pub fn set_loaded(&mut self){
        self.loaded = true;
        log::info!("Wave fully loaded: {} / {:?}", self.len, self.theoric_len);
    }
}
...
/// - Output 0: wave
#[duplicate_item(
    f48       WaveStream48       WaveStream48Player;
    [ f64 ]   [ WaveStream64 ]   [ WaveStream64Player ];
    [ f32 ]   [ WaveStream32 ]   [ WaveStream32Player ];
)]
#[derive(Clone)]
pub struct WaveStream48Player<T: Float> {
    wave: Option<Arc<tokio::sync::RwLock<WaveStream48>>>,
    //channel: usize,
    index: usize,
    start_point: usize,
    end_point: usize,
...
pub enum WaveStreamPlayerPositionError{
    OutOfRange,
    NotFullyLoaded,
    NoWave,
    NoSampleRate
}
impl std::fmt::Display for WaveStreamPlayerPositionError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            WaveStreamPlayerPositionError::OutOfRange => write!(f, "OutOfRange"),
            WaveStreamPlayerPositionError::NotFullyLoaded => write!(f, "NotFullyLoaded"),
            WaveStreamPlayerPositionError::NoWave => write!(f, "NoWave"),
            WaveStreamPlayerPositionError::NoSampleRate => write!(f, "NoSampleRate"),
        }
    }
}
...
impl<T: Float> WaveStream48Player<T> {
    pub fn new(
        wave: Option<Arc<tokio::sync::RwLock<WaveStream48>>>,
        //channel: usize,
        start_point: usize,
    ...
    pub fn set_loop(&mut self, loop_point: Option<usize>){
        self.loop_point = loop_point;
    }

    pub fn set_wave(&mut self, wave: Arc<tokio::sync::RwLock<WaveStream48>>, repeat: bool){
        self.sample_rate = Some(wave.blocking_read().sample_rate());
        self.wave = Some(wave);
        self.index = 0;
        self.loop_point = if repeat{
            Some(0)
        }else{
            None
        }
    ...
    pub fn set_position(&mut self, position: f64) -> Result<(), WaveStreamPlayerPositionError/*String*/>{
        if let Some(sr) = self.sample_rate{
            let next_position: usize = (sr * position) as usize;
            if let Some(wave) = &self.wave{
                let len = wave.blocking_read().len();
                if len > next_position{
                    self.index = next_position;
                    Ok(())
                }else if wave.blocking_read().loaded{
                    Err(WaveStreamPlayerPositionError::OutOfRange)
                    //Err("position out of range".to_string())
                }
                else{
                    Err(WaveStreamPlayerPositionError::NotFullyLoaded)
                    //Err("File not fully loaded".to_string())
                }
            }
            else{
                Err(WaveStreamPlayerPositionError::NoWave)
                //Err("no wave loaded".to_string())
            }
        }else{
            Err(WaveStreamPlayerPositionError::NoSampleRate)
...
impl<T: Float> AudioNode for WaveStream48Player<T> {
    const ID: u64 = 65;
    type Sample = T;
    ...
    fn tick(
        ...
        if let Some(wave) = self.wave.as_mut(){
            let w = wave.blocking_read();
            self.end_point = w.length();
            if self.index < self.end_point {
                let value_left = w.at(0, self.index);
                let value_right = if w.channels() > 1{
                    w.at(1, self.index)
                }else{
                    value_left
                };
                ...
                [convert(value_left), convert(value_right)].into()
            } else {
                [T::zero(), T::zero()].into()
            }
        }
